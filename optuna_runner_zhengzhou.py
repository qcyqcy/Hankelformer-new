# optuna_runner_zhengzhou.py (修复MKL问题版本)
import optuna
import subprocess
import sys
import os
...
def run_experiment(params, config):
    ...
    cmd = [
        ...
        "--model_id", f"zhengzhou_{params['pred_len']}_trial_{params.get('trial_id', 'test')}",
        ...
        "--train_epochs", config.get('train_epochs', '20'),
        "--patience", "3",
        "--use_norm", "1"
    ]
    
    print(f"Running trial {params.get('trial_id', 'test')} with params: ws={params['window_size']}, cw={params['contrastive_weight']:.6f}, lr={params['learning_rate']:.6f}")
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python optuna_runner_zhengzhou.py <config_file>")
        sys.exit(1)
