...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # 计算每个片段的大小和最后一个片段的大小
        self.segment_size = self.pred_len // 8
        self.last_segment_size = self.pred_len - 7 * self.segment_size  # 处理余数
        ...
        # 修改线性层定义，使用segment_size代替pred_len // 8
        self.out_linear_1 = torch.nn.Linear(self.d_model, self.segment_size)
        self.out_linear_2 = torch.nn.Linear(
            self.d_model + self.segment_size, self.segment_size
        )
        self.out_linear_3 = torch.nn.Linear(
            self.d_model + 2 * self.segment_size, self.segment_size
        )
        self.out_linear_4 = torch.nn.Linear(
            self.d_model + 3 * self.segment_size, self.segment_size
        )
        self.out_linear_5 = torch.nn.Linear(
            self.d_model + 4 * self.segment_size, self.segment_size
        )
        self.out_linear_6 = torch.nn.Linear(
            self.d_model + 5 * self.segment_size, self.segment_size
        )
        self.out_linear_7 = torch.nn.Linear(
            self.d_model + 6 * self.segment_size, self.segment_size
        )
        # 最后一个线性层特别处理，输出尺寸为last_segment_size
        self.out_linear_8 = torch.nn.Linear(
            self.d_model + 7 * self.segment_size, self.last_segment_size
        )
